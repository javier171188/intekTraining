...
const model = (function () {
  ...
  function init () {
    ...
    // This is more versatile than the pre-existing updateNotes function
    ...
    function saveNoteDataBase (obj) {
      const data = readStoreItem('notes', '{}')
      const dbid = obj.dbid
      delete obj.dbid
      data[dbid] = obj
  ...
  function init () {
    ...
    function start () {
      ...
      function editClickedLogger (topic, dbid) {
        const dates = presenter.dates(dbid)
        const activeNotes = presenter.data
